...
from judy.B20220809.disk_parsing import Disk
...
class AdminDB:
    ...
    def __init__(self, node: Node, cpu: CPU, gpu: Gpu, disk: Disk) -> None:
        self.node = node
        self.cpu = cpu
        self.gpu = gpu
        self. disk = disk
    ...
    def mergeKey(self):
        ...
        # cpu
        self.cpu_table = cpu.get_fixed_cpu_info()

        cpu_name['cpu_name'] = self.cpu_table.get('cpu_name')

        # node_fixed
        self.node_fixed_table.update(node.get_node_fixed_info())
        self.node_fixed_table.update(disk.get_node_fixed_disk_info())
        self.node_fixed_table.update(gpu.get_node_fixed_gpu_info())
        self.node_fixed_table.update(cpu_name)

        ip['ip'] = self.node_fixed_table.get('ip')

        # node_changed
        self.node_change_table.update(node.get_node_changing_info())
        self.node_change_table.update(cpu.get_changed_cpu_info())
        self.node_change_table.update(disk.get_node_change_disk_info())
        self.node_change_table.update(ip)

        # disk_fixed
        self.disk_fixed_table = disk.get_disk_fixed_list()
        for l in self.disk_fixed_table:
            l.update(ip)
            disk_path.append(l['disk_path'])  # disk_change에서 쓰는 FK 추출

        # disk_change
        self.disk_changed_table = disk.get_disk_change_list()
        ...
        # gpu_fixed
        self.gpu_fixed_table = gpu.get_gpu_fixed_list()
        ...
        # gpu_change
        self.gpu_changed_table = gpu.get_gpu_change_list()
    ...
    def makeSQL(self, table: str, data: dict):
        # 넣을 data column
        columns_str: str = "("
...
if __name__ == "__main__":
    ...
    node = Node()
    cpu = CPU()  # 이상한게 cpu의 경우 changed_info가 최초실행일때만 값이 들어가고 그 뒤엥 0.0 100.0으로 됨 why?
    gpu = Gpu()
    disk = Disk()

    admindb = AdminDB(node, cpu, gpu, disk)
