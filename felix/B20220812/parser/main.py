...
class AdminDB:
    ...
    def __init__(self, ssh, cur = None, conn = None):
        # 원격 접속을 위한 ssh
        self.ssh = ssh
...
if __name__ == "__main__":
    ...
    conn = pymysql.connect(host='localhost', user='root',
                                password='baro', db='Felix', charset='utf8')
    cur = conn.cursor()
    ...
    admindb = AdminDB(ssh, cur, conn)

    admindb.mergeFixedKey()
    admindb.mergeChangeKey()

    print(admindb.gpu_changed_table)
    print(admindb.disk_changed_table)
