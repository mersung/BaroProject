...
if __name__ == "__main__":
    ...
    conn = pymysql.connect(host='localhost', user='root',
                                password='baro', db='bumstead', charset='utf8')
    cur = conn.cursor()
    ...
    ssh_114 = paramiko.SSHClient()
    ssh_114.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh_114.connect("192.168.20.114", port='22',  # 고객이 자신의 ip를 안다고 가정하에 '115'부분을 바꿈, 그러면 고객의 node에서 115로 가져올 수 있음, 지금은 115로 자기자신과 연결
                username="oem", password='baro')  # customer

    admindb = AdminDB(conn, cur, ssh)
    admin_114 = AdminDB(conn, cur, ssh_114)
    ...
    try:
        ...
        admin_114.fixed_insert_db()
        while True:
            admindb.changed_insert_db()
            admin_114.changed_insert_db()
            time.sleep(2)
